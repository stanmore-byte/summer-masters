...
// ── ONE-TIME FIX: Hannah June 6 ─────────────────────────────────
// Sets all Ball Skills + Strength Training complete for June 6
// Select fixHannahJune6 in dropdown → click Run
function fixHannahJune6() {
  const ss   = SpreadsheetApp.openById(SPREADSHEET_ID);
  ...
  for (let i = 1; i < dRows.length; i++) {
    if (String(dRows[i][0]).trim() === 'Hannah' && fmtDate(dRows[i][1]) === date) {
      ...
      found = true;
      Logger.log('Updated Hannah row for ' + date);
      break;
    }
  }
  if (!found) {
